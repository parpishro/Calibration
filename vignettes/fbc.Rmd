...
```{r summary and print}
summary(calMod) # print(calMod) has exactly the same output
```
...
Elements of vector $\kappa$ are parameters of the calibration model and will be estimated by `calibrate()` [^40]. 

\

[^40]: *The first $q$ columns of matrix `Phi` in `calibrate` output represent the MCMC samples of posterior densities for calibration parameters. In our ball example, there is only one calibration parameter (gravity), which is denoted by $\kappa_1$ and represented by `kappa1` in `Phi`.*</em>.
...
| $p$               | Number of experimental inputs                                    | 1                                          |
|                   |                                                                  |                                            |
| $\bf{x}_f$        | Field input vector containing $p$ experimental inputs            | $h$ (scaler)[^50]                          |
|                   |                                                                  |                                            |
| $\kappa$          | Vector of unknown true calibration inputs in field experiment    | true value of gravity $\kappa_1$           |
|                   |                                                                  |                                            |
| $\bf{x}_{\kappa}$ | Augmented field input vector containing $(p+q)$ inputs[^45]       | ($h$, $\kappa_1$)$^2$ (vector of length 2) |
|                   |                                                                  |                                            |
| $y_f$             | Univariate field response                                        | $t$ (scaler)                               |
|                   |                                                                  |                                            |
| $\bf{X}_f$        | $(n \times p)$ field input matrix                                | $\bf{h}$ (vector$^2$ of length $63$)       |
|                   |                                                                  |                                            |
| $\bf{X}_{\kappa}$ | $(n \times (p+q))$ augmented field input matrix                  | $[\bf{h} \quad \bf{\kappa_1}]$[^55]              |
|                   |                                                                  |                                            |
| $\bf{y}_f$        | Vector of $n$ univariate field response                          | $\bf{t}$ (vector of length $63$)           |
|                   |                                                                  |                                            |

[^50]: *In ball example there is only one experimental input and therefore $\bf{x}_f$ is a vector of length one or scaler. Similarly, in its matrix notation, $\bf{X}_f$ is a $(n \times 1)$ matrix or a vector of length $n$.*</em>.
[^45]: *Note that calibration parameters $\bf{\kappa}$ are often assumed to be unchanged throughout field experiment. Therefore, same ($\kappa$) vector is augmented to all of the field input configurations.*</em>.
[^55]: *Since calibration input is fixed for all field observation, $\bf{\kappa_1} = (\kappa_1, ... , \kappa_1)$ vector of length 63} ] ($(100 \times 2)$ matrix).*</em>.
